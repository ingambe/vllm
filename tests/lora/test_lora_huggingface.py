...
def test_load_local_dora_checkpoint(tmp_path):
    adapter_dir = tmp_path / "local_dora"
    adapter_dir.mkdir()
    config = {
        "r": 2,
        "lora_alpha": 4,
    ...
    peft_helper = PEFTHelper.from_local_dir(adapter_dir, 128)
    lora_model = LoRAModel.from_local_checkpoint(
        adapter_dir,
    ...
    assert lora_model.loras["dense"].magnitude_vector is not None
